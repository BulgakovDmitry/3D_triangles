...
public:
    ...
    void attach(GLFWwindow* window) {
        ...
        prev_framebuffer_cb_  = glfwSetFramebufferSizeCallback(window_, &OrbitCamera::framebuffer_trampoline);
...
private:
    ...
    static void cursor_trampoline(GLFWwindow* w, double x, double y) {
        if (auto* self = from(w)) self->on_cursor(x, y);
        if (auto* self = from(w); self && self->prev_cursor_cb_) self->prev_cursor_cb_(w, x, y);
    }
    static void scroll_trampoline(GLFWwindow* w, double xo, double yo) {
        if (auto* self = from(w)) self->on_scroll(xo, yo);
        if (auto* self = from(w); self && self->prev_scroll_cb_) self->prev_scroll_cb_(w, xo, yo);
    }
    static void button_trampoline(GLFWwindow* w, int b, int a, int m) {
        if (auto* self = from(w)) self->on_button(b, a, m);
        if (auto* self = from(w); self && self->prev_button_cb_) self->prev_button_cb_(w, b, a, m);
    }
    static void framebuffer_trampoline(GLFWwindow* w, int ww, int hh) {
        if (auto* self = from(w)) self->on_framebuffer_resized(ww, hh);
        if (auto* self = from(w); self && self->prev_framebuffer_cb_) self->prev_framebuffer_cb_(w, ww, hh);
    }

    // ----- обработчики -----
    void on_cursor(double x, double y) {
        if (!rotating_) { last_ = {x, y}; return; }
    ...
    struct Cursor { double x{0.0}, y{0.0}; };
