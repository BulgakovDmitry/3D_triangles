...
void BVH::dump_graph() const {
    ...
    if (!gv) throw std::runtime_error("open gv file - error");
...
void BVH::dump_graph_list_nodes(const std::unique_ptr<Node>& node, std::ofstream& gv) const {
    ...
    if (!node->is_branch()) {
        ...
        gv << "    node_" << node.get()
           << " [shape=Mrecord; style=filled; fillcolor=palegreen; color=\"#000000\"; fontcolor=\"#000000\"; "
           <<  "label=\"{ node_" << node.get() << " | p_min (" << pmin.get_x() << ", " << pmin.get_y() << ", " << pmin.get_z()
           <<  ") | p_max (" << pmax.get_x() << ", " << pmax.get_y() << ", " << pmax.get_z()
           <<  ") | { left: " << node->get_left().get() << " | right: " << node->get_right().get() << " } }\""
           << "];\n";
    } else {
        gv << "    node_" << node.get()
           << " [shape=Mrecord; style=filled; fillcolor=cornflowerblue; color=\"#000000\"; fontcolor=\"#000000\"; "
           <<  "label=\"{ node_" << node.get() << " | p_min (" << pmin.get_x() << ", " << pmin.get_y() << ", " << pmin.get_z()
           <<  ") | p_max (" << pmax.get_x() << ", " << pmax.get_y() << ", " << pmax.get_z()
    ...
    if (node->get_left())  dump_graph_list_nodes(node->get_left(),  gv);
    if (node->get_right()) dump_graph_list_nodes(node->get_right(), gv);
}
...
void BVH::dump_graph_connect_nodes(const std::unique_ptr<Node>& node, std::ofstream& gv) const {
    ...
    if (node->get_left())  dump_graph_connect_nodes(node->get_left(),  gv);
    if (node->get_right()) dump_graph_connect_nodes(node->get_right(), gv);
}
