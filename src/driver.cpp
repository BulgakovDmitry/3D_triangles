...
static std::vector<std::size_t> get_numbers_of_intersecting_triangles  (const std::vector<Triangle>& triangles);
...
static std::vector<std::size_t> get_numbers_of_intersecting_triangles(const std::vector<Triangle>& triangles) {
    ...
    for (std::size_t i = 0; i < N; ++i)
        if (hit[i]) numbers_of_intersecting_triangles.push_back(i + 1); 
